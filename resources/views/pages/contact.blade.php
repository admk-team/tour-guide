<?php
$about_us      = \App\Models\LandingPage\LandingPageKey::where('key', 'About Us')->first();
$landingPageKey = \App\Models\LandingPage\LandingPageKey::where('key', 'Contact Us')->first();

if ($landingPageKey) {
    // If the record is found, retrieve its related contents
    $contents = $landingPageKey;

    if ($contents) {
        // If contents are found, retrieve the 'title' attribute
        $contactTitle = $contents->title;

        // Retrieve the image URL attribute (assuming it's stored in a 'image_url' column)
        $contactImageUrl = $contents->contents;
    } else {
        // Handle the case where contents are not found
        $contactTitle = null;
        $contactImageUrl = null;
    }

} else {
    // Handle the case where the LandingPageKey is not found
    $contactTitle = null;
    $contactImageUrl = null;
}
...
@section('content')
    ...
    <section style="height: 30rem">
        ...
        <!--<img src="{{ asset('images/guides-navigator-contact-us.jpg?v=1') }}" alt="Contact Image" class="w-full h-full object-cover">-->
        <img src="{{ $landingPageKey->contents()->latest()->first()->contact_us ?? asset('images/contact_us.jpeg') }}" alt="" class="w-full h-full object-cover">
        <!--<img src="{{ $about_us->contents()->latest()->first()->about_us[0] ?? asset('images/abouts-4.jpeg') }}" alt="" class="w-full h-full object-cover">-->
