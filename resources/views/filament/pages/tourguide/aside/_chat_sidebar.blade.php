                ...
                <div class="flex justify-between py-4 border-gray-200 cursor-pointer
                    ...
                    <div class="lg:flex flex-col items-center space-y-4 mr-5">
                        @if($reciver->messages()->whereRoomId($room->id)->exists())
                            ...
                            <!--    {{--{{ $sideRoom->memberable->messages()->whereRoomId($room->id)->latest()->first()->created_at->diffForHumans() }}--}}-->
                            <!--    </span>-->
                            <span class="text-xs text-gray-500">
                                @if ($sideRoom && $sideRoom->memberable && $room)
                                    @if ($sideRoom->memberable->messages()->whereRoomId($room->id)->exists())
                                        {{ $sideRoom->memberable->messages()->whereRoomId($room->id)->latest()->first()->created_at->diffForHumans() }}
                                    @endif
                                @else
