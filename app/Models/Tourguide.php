...
class Tourguide extends Authenticatable implements HasName, HasMedia, HasAvatar
{
    ...
    public function availableForBooking($event_id, $start, $end): bool
    {
        $event = Event::find($event_id);
        
        // Check if the event is in the future
        if ($event->start_at <= now()) {
            return false; // Event has already started, can't book
        }
    
        // Check if there are overlapping appointments for this tour guide
        if ($this->hasOverlappingAppointments($start, $end)) {
            return false; // There is an overlap, the tour guide is not available
        }
    
        // Check if the tour guide can be assigned to this type of event
        if (!$this->canAssignToEvent($event_id)) {
            return false; // Tour guide can't be assigned to this type of event
        }
    
        // Check if the tour guide has not been booked by the agent for this event
        return $this->doesntBookedFromAgent(auth('agent')->id(), $event_id);
    }
    
    public function hasOverlappingAppointments($start, $end)
    {
