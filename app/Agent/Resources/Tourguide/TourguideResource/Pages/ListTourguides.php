...
//          'education' => $this->education,
    ...
    protected function getFormSchema(): array
    {
        return [
            ...
            //dd(__('attributes.years', ['field' => '3'])),
...
//          ->when($this->have_a_photo, fn($query) => $query->whereHas('media', fn($query) => $query->where('collection_name', 'tourguide_avatar')))
            ...
            ->when($this->years_of_experience, fn($query) => $query->where('years_of_experience', '>=', $this->years_of_experience))

            // ->when($this->years_of_experience, fn($query) => $query->whereHas('work_experiences', function ($query) {
            //     //dd($this->work_experiences);
            ...
            //->when($this->skills, fn($query) => $query->whereIn('id', $this->skills))
             ->when($this->skills, function ($query) {
            $query->whereHas('skills', function ($query) {
                $query->whereIn('skills.id', $this->skills); // Specify 'skills.id' to avoid ambiguity
                });
            })
            ->when($this->languages, function ($query) {
                $query->whereHas('languages', function ($query) {
                    $query->whereIn('languages.id', $this->languages); // Specify 'skills.id' to avoid ambiguity
                });
            })
